...
class TestLoadMRC:
    ...
    def test_load_ali(self):
        filename = os.path.join(etspy_path, "tests", "test_data", "HAADF.ali")
        stack_orig = hs.load(filename)
        stack = etspy.io.load(filename)
    ...
    def test_load_mrc_with_bad_rawtlt(self):
        ...
        stack.data = np.append(
            stack.data, np.zeros([1, stack.data.shape[1], stack.data.shape[2]]), axis=0
        )
...
class TestHspy:
    ...
    def test_load_hspy_hdf5(self):
        filename = os.path.join(etspy_path, "tests", "test_data", "HAADF_Aligned.hdf5")
...
class TestDM:
    def test_load_single_dm(self):
        filename = os.path.join(etspy_path, "tests", "test_data", "HAADF.dm3")
    ...
    def test_load_dm_series(self):
        dirname = os.path.join(etspy_path, "tests", "test_data", "DM_Series_Test")
...
class TestUnknown:
    def test_load_unknown_string(self):
        filename = os.path.join(etspy_path, "tests", "test_data", "HAADF.NONE")
