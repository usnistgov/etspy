...
class TestLoadMRC:
    ...
    def test_load_mrc(self):
        ...
        assert isinstance(stack.metadata.Tomography.tilts, np.ndarray)
        assert stack.metadata.Tomography.tilts.shape[0] == stack.data.shape[0]
        assert stack.metadata.Tomography.tilts[0] == -76  # noqa: PLR2004
...
class TestHspy:
    ...
    def test_convert_signal2d(self):
        ...
        assert isinstance(stack.metadata.Tomography.tilts, np.ndarray)
        assert stack.metadata.Tomography.tilts.shape[0] == stack.data.shape[0]
        assert stack.metadata.Tomography.tilts[0] == -10  # noqa: PLR2004
        assert isinstance(stack, TomoStack)
...
class TestNumpy:
    ...
    def test_numpy_to_stack_with_tilts(self):
        ...
        assert stack.metadata.has_item("Tomography")
        assert isinstance(stack.metadata.Tomography.tilts, np.ndarray)
        assert stack.metadata.Tomography.tilts[0] == -50  # noqa: PLR2004
        assert stack.metadata.Tomography.tilts.shape[0] == stack.data.shape[0]
        assert isinstance(stack, TomoStack)
