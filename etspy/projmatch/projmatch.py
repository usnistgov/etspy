...
class ProjMatch:
    ...
    def apply_shifts(
        ...
        """
        shifted = self.stack.deepcopy()

        if len(self.total_shifts) != self.stack.data.shape[0]:
            msg = (
                f"Number of shifts ({len(self.total_shifts)}) is not consistent "
                f"with number of images in the stack ({self.nangles})"
            )
